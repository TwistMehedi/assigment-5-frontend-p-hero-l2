...
const MovieDetailsPage = () => {
  ...
  const { data: checkResponse, isLoading: isCheckLoading } = useCheckPurchaseQuery(id as string, {
    skip: !id,
  });
  ...
  if (isError || !movie) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
        ...
        <h2 className="text-2xl font-bold uppercase tracking-tighter italic">Movie not found!</h2>
    ...
    <div className="min-h-screen pb-20 bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 mt-[-200px] relative z-10">
        <div className="flex flex-col md:flex-row gap-10">
           ...
           <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} className="w-full md:w-1/3 shrink-0">
            ...
            <div className="mt-6 space-y-4">
              <Button
                ...
                className={`w-full h-14 rounded-2xl font-black uppercase italic tracking-tighter gap-3 transition-all ${
                  isPurchased ? "bg-green-600 hover:bg-green-700 text-white" : "bg-primary text-black hover:bg-primary/90"
                }`}
              >
                {isCheckLoading ? (
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4 text-sm font-bold">
                ...
                <span className="flex items-center gap-1 bg-muted px-3 py-1 rounded-full text-muted-foreground uppercase">
                  <Calendar className="h-4 w-4" /> {new Date(movie.releaseDate).getFullYear()}
                </span>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed max-w-3xl">{movie.description}</p>
            </motion.div>

             <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="bg-card p-8 rounded-3xl border border-border shadow-sm space-y-6 mt-12">
              ...
              <textarea className="w-full h-32 p-4 bg-background border border-border rounded-2xl outline-none" placeholder="Share your thoughts..." />
              <Button variant="outline" className="font-black border-primary text-primary hover:bg-primary hover:text-white uppercase tracking-widest h-12 px-8">
                Submit Review
              </Button>
            </motion.div>
          </div>
